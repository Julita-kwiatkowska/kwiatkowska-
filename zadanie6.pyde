...
class KolorowyKwadrat(Kwadrat):
    def sketchPasiasty(self, x, y, paski_paski): # dodajemy ilość pasków, w które ma być kwadrat
        Kwadrat.sketch(self, x, y) # korzystamy z metody klasy bazowej
        space = self.bok/paski_paski # wyliczamy przerwęmiędzy paskami
        _xLinii_ = 0 # to jest pole chronione, nie powinno się go używać w kodzie po za klaą i klasami po niej dziedziczącymi 
        for pasek in range(0, paski_paski/2):
            line(x+_xLinii_, y, x+_xLinii_, y+self.bok/2)
        for pasek in range(1, paski_paski): # dorysowujemy paski
            line(x+_xLinii_, y, x+_xLinii_, y+self.bok/2)
            _xLinii_ +=space       
